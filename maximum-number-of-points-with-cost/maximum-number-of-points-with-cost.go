...
func maxPoints(points [][]int) int64 {
	...
	for i := 1; i < len(points); i++ {
		...
		for k := 1; k < len(points[i]); k++ {
			...
			fmt.Println("dp[i-1][k]+k", dp[i-1][k]+k)
			...
			fmt.Println("left_dpPPP", left_dp)
			// fmt.Println("---")
		...
		for j := 0; j < len(points[i]); j++ {
			fmt.Println("-***55555**-")
			fmt.Println("left_dp[j]-j", left_dp[j]-j)
			fmt.Println("right_dp[j]+j", right_dp[j]+j)
